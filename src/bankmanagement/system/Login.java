...
public class Login extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == clear){
           ...
           String pinnumber = pinTextFeild.getText();   //its occurs due to pin is password 
           String query = "select * from login where cardnumber = '"+cardnumber+"' and pin = '"+pinnumber+"'";
           try{
              ResultSet rs = conn.s.executeQuery(query);   //stores the result data in it from database
              if(rs.next()){
                  setVisible(false);
            ...
            new SignupOne().setVisible(true); //to shift login frame to signupone frame
